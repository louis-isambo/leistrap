...
function dollarEvent(widget, ls) {
    ...
    // add an new Element
    append: function (objc, element, func, privProp) {

        if (objc._conf) {
            element = element.point ? element.point : element;
            objc._conf.append(element.render());
            element.state = "active";
            element.parent = objc._conf; element.lsParent = objc;
            objc.content.push(element);
    ...
    whenAutoItemClicked: function (item, option, input, card) {
        item.addEvent("click", () => {
            this.whenSelectAutoComplete(option.whenSelect, item.text, input);
    ...
    setAutoCompletion: function (objClss, list, input, defaultValue, option) {
        ...
        function updateAutoCompletion(auto, values, c) {
            const item = objClss.Li({ text: values });
            leis$1.whenAutoItemClicked(item, option, input, c);
            auto.addItem(item);
        }
        ...
        if (!obj$1.isUndifend(defaultValue)) {
            ...
            defaultValue.forEach(item => {
                const it = objClss.Li({ text: item });
                this.whenAutoItemClicked(it, option, input, c);
                GI.addItem(it);
            });
        }
        ...
        input.addEvent("keyup", function (e) {
            ...
            c.getScreen(); GI.removeAll();
            var typed = this.getAttr("value") ? this.getAttr("value").toLowerCase() : "";
            list.forEach(item => {

                if (obj$1.has(typed, item.toLowerCase())
                    && !obj$1.isEmpty(typed)) {
                    // by one result
...
class Content extends Array {
    ...
    constructor(o, ...item) {
        ...
        this.add(item);
        // add an index
        this.forEach(item => { if (item.index) { this.addIndex(item); } });
    }
...
const BaseElement = (function () {
    ...
    class LeisWidget {
         ...
         */
        addElements(...elements) { leis$1.appendElement(this, ...elements); }
        ...
        setStyleProp(prop, value) {
            if (leis$1.hasConf(this)) { leis$1.setStyleProp(this, prop, value); }
            else {
...
class Modal {
    ...
    #setModal() {
        ...
        _modal_.addEvent("click", function (e) {
            if (e.target === this._conf) { this.removeClass("show"); }
        });

        leis$1.addClassList(_modal_, "leis-modal-container");
        leis$1.addClassList(dialog, "leis-modal-dialog");
...
leistrap.defineExtension = function (name, extn, option) {
    this[name] = extn(setting, leistrap, extensionOption);
...
leistrap.createContent = function (elem, n, clb) {
    if (obj$1.isFunction(clb)) {
        return this.inRange(n, 0, (item, index) => clb(this[elem](), index));
    }
    else {
